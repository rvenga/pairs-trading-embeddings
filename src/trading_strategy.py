...
def generate_signals(z_score, entry_threshold=2.0, exit_threshold=0.5, stop_loss=3.5):
    ...
    for i in range(len(z_score)):
        ...
        if pd.isna(z):
            # FIX: Reset position when we hit NaN to maintain state consistency
            position = 0
            signals.iloc[i] = 0
            continue
...
def calculate_pnl(price1, price2, hedge_ratio, signals, initial_capital=1.0):
    ...
    FIXED VERSION: This correctly implements dollar-neutral pairs trading.
    
    For a dollar-neutral pairs trade:
    - Total capital = initial_capital
    - When signal = 1 (long spread):
        * Long stock1 with weight = 1 / (1 + hedge_ratio)
        * Short stock2 with weight = hedge_ratio / (1 + hedge_ratio)
    - When signal = -1 (short spread): flip the signs
    ...
    # Calculate position weights for dollar neutrality
    ...
    # Total daily P&L = sum of both legs
